...
| Signal integrity / length matching (S14) | `checks/{signal-integrity,diff-pair-paths,length}.ts`, `src/shared/drc/si/{coupled-span,length-target}.ts`, `src/shared/drc/diff-pair-resolver.ts`, the path model `src/shared/pcb-connectivity/{junctions,contact-components,terminal-contact,island-contact,trace-arc,net-path,net-path-graph,net-path-uniqueness,net-path-types}.ts`, `src/shared/pcb-geometry/segment-sublevel.ts`, the frontend consumers `src/modules/designer/frontend/pcb/{use-net-path-lengths.ts,tools/diff-pair.ts}`; contract `docs/pcb-hardening/14-si-contract.md`; every future SI capability extends it under the binding rules of `docs/pcb-hardening/15-high-speed-runway.md` §3 (the junction graph is the input, never the `NetPath` scalars; join, never rename; broadside / stitched copper get new measures) and the stack-up brief §4 (owner S15b) | ✅✅ |
...
>   private answers to "is this copper connected": island anchoring reads the component (S5), routed length and coupling read the path model over the S1 junctions (S14 — `computeConnectivity(items, { junctions: true })`, `net-path.ts`; contract 14), the routing-obstacle and live-DRC pad layers read the `LegalityContext` items (S8). Never add another.
