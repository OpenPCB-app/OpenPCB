 ...
 */
export const designerModule = createModuleV2("designer", {
    label: "Designer",
    namespace: "space.designer",
    version: "0.1.0",
    kind: "space",
    spaceComponent: DesignerSpace,
    ...
    endpoints(ctx, http, ws) {
        ...
        http.get("/example", async (req) => {
            ...
            return new Response(JSON.stringify({
                message: `Hello, ${name} from Designer!`,
    ...
    onActivate: async (ctx) => {
        ctx.logger.info("Designer module activated");
    },

    onDeactivate: async (ctx) => {
        ctx.logger.info("Designer module deactivated");
    },

    // Services (for service/integration modules)
    // services: (ctx) => ({
    //     "space.designer.exampleService": async (input: unknown) => {
