 ...
 */
export const aiServiceModule = createModuleV2("ai-service", {
    label: "Ai Service",
    namespace: "space.aiservice",
    version: "0.1.0",
    kind: "space",
    spaceComponent: AiServiceSpace,
    ...
    endpoints(ctx, http, ws) {
        ...
        http.get("/example", async (req) => {
            ...
            return new Response(JSON.stringify({
                message: `Hello, ${name} from Ai Service!`,
    ...
    onActivate: async (ctx) => {
        ctx.logger.info("Ai Service module activated");
    },

    onDeactivate: async (ctx) => {
        ctx.logger.info("Ai Service module deactivated");
    },

    // Services (for service/integration modules)
    // services: (ctx) => ({
    //     "space.aiservice.exampleService": async (input: unknown) => {
...
export default aiServiceModule;
