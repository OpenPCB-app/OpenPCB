...
async function writeModule(
  ...
  await mkdir(path.join(moduleDir, "core"), { recursive: true });
  await writeFile(path.join(moduleDir, "manifest.json"), JSON.stringify(manifest, null, 2), "utf8");
  await writeFile(path.join(moduleDir, "core", "backend-entry.mjs"), backendEntrySource, "utf8");
}
...
function baseManifest(moduleId: string): Record<string, unknown> {
  return {
    ...
    runtime: {
      backendEntry: "core/backend-entry.mjs",
    },
    dependsOn: [],
  };
}
...
describe("module runtime bootstrap", () => {
  ...
  test("exposes registry snapshot and module routes through HTTP server", async () => {
    ...
    const statusBody = (await statusResponse.json()) as { ok: boolean; name: string };
