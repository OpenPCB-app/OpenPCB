...
describe("diagnostics endpoint", () => {
  test("returns minimal diagnostics payload", async () => {
    ...
    const response = await server.fetch(new Request("http://localhost/api/diagnostics"));
    const body = (await response.json()) as {
      ok: boolean;
      data: { startedAt: string; uptimeMs: number; errorCount: number; recentErrors: unknown[] };
    };
