...
describe("component-library native route migration", () => {
  test("serves native parts/symbols/footprints routes and registers SDK", async () => {
    ...
    expect(sdkRegistry.has(MODULE_SDK_TOKENS.CORE_PROJECTS)).toBe(true);

    const componentLibrarySdk = sdkRegistry.resolve<ComponentLibrarySDK>(
      MODULE_SDK_TOKENS.COMPONENT_LIBRARY,
    );
    const projectsCapability = sdkRegistry.resolve<ProjectsCapability>(
      MODULE_SDK_TOKENS.CORE_PROJECTS,
    );

    const parts = await componentLibrarySdk.searchParts({ query: "capacitor", limit: 20 });
    expect(parts.length).toBeGreaterThan(0);
    const resolved = await componentLibrarySdk.resolvePart(parts[0]!.id);
    expect(resolved?.id).toBe(parts[0]!.id);

    const projects = await projectsCapability.list({ workspaceId: "workspace-test", status: "all" });
    ...
    const searchResponse = await server.fetch(
      new Request("http://localhost/api/modules/component-library/parts?q=resistor"),
    );
    expect(searchResponse.status).toBe(200);
    const searchBody = (await searchResponse.json()) as {
      data?: { parts?: Array<{ id: string; symbolId: string; footprintId: string }> };
    };
    const first = searchBody.data?.parts?.[0];
    expect(first?.id).toBeDefined();

    const symbolResponse = await server.fetch(
      new Request(`http://localhost/api/modules/component-library/symbols/${first?.symbolId}`),
    );
    expect(symbolResponse.status).toBe(200);

    const footprintResponse = await server.fetch(
      new Request(`http://localhost/api/modules/component-library/footprints/${first?.footprintId}`),
    );
