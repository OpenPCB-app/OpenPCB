...
import type { RequestContext } from "../runtime/http/request-context";
import { success } from "../runtime/http/response";
import { HttpRouter } from "../runtime/router/http-router";
import { ModuleRouter } from "../runtime/router/module-router";
import { ModuleRouterRegistry } from "../runtime/router/module-registry";
import { RouteParams } from "../runtime/router/route-params";
...
describe("runtime router behavior", () => {
  ...
  test("preserves module query string during dispatch", async () => {
    ...
    moduleRouter.get("/items", async (ctx) => success({ limit: ctx.query.get("limit") }));
    ...
    const body = (await response.json()) as { ok: boolean; data?: { limit: string | null } };
  ...
  test("prefers static routes over dynamic routes", async () => {
    ...
    router.get("/items/:itemId", async (ctx) => success({ route: "dynamic", value: ctx.params.getOrThrow("itemId") }));
    router.get("/items/new", async () => success({ route: "static" }));

    const response = await router.dispatch(createContext("http://localhost/items/new"));
  ...
  test("keeps request body readable after body validation", async () => {
    ...
    router.post(
      ...
      async (ctx) => {
        ...
        return success({ rawName: rawBody.name, validatedName: validatedBody.name });
      },
    ...
    const body = (await response.json()) as { data?: { rawName: string; validatedName: string } };
  ...
  test("preserves repeated query params for zod validation", async () => {
    ...
    const response = await router.dispatch(createContext("http://localhost/search?tag=a&tag=b"));
    const body = (await response.json()) as { data?: { tags: { tag: string[] } } };
  ...
  test("passes matched params to module error boundaries", async () => {
    ...
    const response = await moduleRouter.dispatch(createContext("http://localhost/items/42"));
