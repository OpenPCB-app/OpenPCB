...
export class RedoUsecase {
  constructor(private deps: RedoDeps) {}
  ...
  async execute(designId: string, sessionId: string): Promise<CommandSuccessResult | null> {
    return this.deps.transactionRunner.runInTransaction(async () => {
      const targetCommandId = this.deps.undoRegistry.peekRedo(designId, sessionId);
      if (!targetCommandId) {
        return null;
      }
      ...
      if (latest && latest.sessionId !== sessionId) {
        throw new ValidationError("Cannot redo after changes from another session");
      }
      ...
      const forwardPatches = structuredClone(target.forwardPatches);
        ...
        logEntry(
          {
            ...
            forwardPatches,
            inversePatches: structuredClone(target.inversePatches),
      ...
      this.deps.undoRegistry.popRedo(designId, sessionId);
      this.deps.undoRegistry.pushUndo(designId, sessionId, targetCommandId);
