...
function segmentToSegmentClosestPoints(
  ...
  const proj = (
    ...
    const pr = projectPointToSegment(point, s0, s1);
    const foot = { x: pr.x, y: pr.y };
    return onAb
      ? { distance: pr.distance, p: point, q: foot }
      : { distance: pr.distance, p: foot, q: point };
  };
  let best = proj(a, c, d, true);
  for (const cand of [
    proj(b, c, d, true),
    proj(c, a, b, false),
    proj(d, a, b, false),
...
/**
 * Like `polylineToPolylineDistance`, but also returns the closest point on each
 * polyline. DRC uses the midpoint of this pair to place the violation marker at
 * the true point of closest approach rather than the midpoint of the bounding
 * boxes.
 */
export function polylineToPolylineClosestPoints(
  a: Point[],
  b: Point[],
): { distance: number; a: Point; b: Point } {
  const fallbackA = a[0] ?? { x: 0, y: 0 };
  const fallbackB = b[0] ?? { x: 0, y: 0 };
  if (a.length < 2 || b.length < 2) {
    return { distance: Infinity, a: fallbackA, b: fallbackB };
  }
  let best = { distance: Infinity, a: fallbackA, b: fallbackB };
  for (let i = 1; i < a.length; i += 1) {
    for (let j = 1; j < b.length; j += 1) {
      const cp = segmentToSegmentClosestPoints(
        a[i - 1]!,
        a[i]!,
        b[j - 1]!,
        b[j]!,
      ...
      if (cp.distance < best.distance) {
        best = { distance: cp.distance, a: cp.p, b: cp.q };
      }
    }
  }
  return best;
