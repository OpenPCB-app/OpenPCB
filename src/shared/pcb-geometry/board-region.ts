...
/** Minimum distance from an open polyline to any ring's perimeter. */
export function regionBoundaryDistancePolyline(
  ...
  for (let s = 1; s < pts.length; s += 1) {
    for (const e of region.edges) {
      const d = segmentToSegmentDistance(pts[s - 1]!, pts[s]!, e.a, e.b);
      if (d < best) best = d;
    }
  }
  return best;
...
/** Minimum distance from a closed ring's perimeter to any ring's perimeter. */
...
export function segmentInsideRegion(
  ...
  if (distance(a, b) <= eps) return regionContainsPoint(region, a, eps);
  if (!regionContainsPoint(region, a, eps)) return false;
  if (!regionContainsPoint(region, b, eps)) return false;
  ...
  for (let i = 1; i < params.length; i += 1) {
    ...
    if (!regionContainsPoint(region, p, eps)) return false;
    prev = t;
  }
  return true;
}
...
export function polylineInsideRegion(
  ...
  if (pts.length === 1) return regionContainsPoint(region, pts[0]!, eps);
  for (let i = 1; i < pts.length; i += 1) {
    if (!segmentInsideRegion(region, pts[i - 1]!, pts[i]!, eps)) return false;
  }
  return true;
}
...
export function polygonInsideRegion(
  ...
  if (canonical.length < 3) return polylineInsideRegion(region, canonical, eps);
  for (let i = 0; i < canonical.length; i += 1) {
    const a = canonical[i]!;
    const b = canonical[(i + 1) % canonical.length]!;
    if (!segmentInsideRegion(region, a, b, eps)) return false;
  }
...
export function stadiumInsideRegion(
  ...
  if (pts.length === 0) return true;
  if (halfWidthMm <= eps) return polylineInsideRegion(region, pts, eps);
  const gap =
    pts.length === 1
      ? regionBoundaryDistancePoint(region, pts[0]!)
      : regionBoundaryDistancePolyline(region, pts);
  return gap >= halfWidthMm - eps && regionContainsPoint(region, pts[0]!, eps);
}
...
export function discInsideRegion(
  ...
  if (radiusMm <= eps) return regionContainsPoint(region, center, eps);
  return (
    regionBoundaryDistancePoint(region, center) >= radiusMm - eps &&
    regionContainsPoint(region, center, eps)
  );
}
