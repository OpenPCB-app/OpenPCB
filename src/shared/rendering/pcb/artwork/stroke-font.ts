 ...
 *
 * It lives in `shared/` (DFM contract 11 §1.2) because the silkscreen artwork
 * model is what BOTH the Gerber writer and the DFM checks read; the export
 * module re-exports it. Glyph geometry is unchanged from the export-only
 * version — only the `anchorY` option is new.
 */
...
/**
 * Vertical anchor, matching `PreviewLabel["anchorY"]`. The shift is defined on
 * the CAP BOX `[0, GLYPH_CAP_HEIGHT]`, never on the ink: `top` hangs the box
 * below the anchor, `bottom` sits it above, `middle` centres it. Three glyphs
 * (`Q`, `,`, `µ`) dip below the baseline, so the two baseline variants cannot
 * be distinguished from `top` / `bottom` by the ink either — they are defined
 * to equal them, which is also what the preview model means by them.
 */
...
export interface TextStrokeOptions {
  /** Anchor point (mm): horizontal per `justify`, vertical per `anchorY`. */
  ...
  /** Defaults to `middle` — the convention every pre-S12 caller assumed. */
  anchorY?: TextAnchorY;
}

/** Cap-box offset (glyph units) that realises `anchorY`. */
function capBoxShift(anchorY: TextAnchorY | undefined): number {
  switch (anchorY) {
    case "top":
    case "top-baseline":
      return -GLYPH_CAP_HEIGHT;
    case "bottom":
    case "bottom-baseline":
      return 0;
    default:
      return -GLYPH_CAP_HEIGHT / 2;
  }
}

/**
 * Vectorize `text` into board-space polylines (mm). Matches the canvas anchor
 * convention (`anchorX = justify`, `anchorY` per the option), then applies
 * rotation and mirror about the anchor.
 */
  ...
  const startX =
    opts.justify === "center"
      ...
      : opts.justify === "right"
        ? -totalUnits
        : 0;
  const yShift = capBoxShift(opts.anchorY);
