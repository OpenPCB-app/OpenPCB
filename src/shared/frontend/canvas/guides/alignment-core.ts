/**
 * Generic placement-alignment guide engine. Operates purely on `{ id, bbox }`
 * boxes (world mm) — no PCB / placement / layer concepts — so it is reused by
 * the PCB editor (placements), the footprint editor (pads + graphics) and the
 * symbol editor (pins + graphics).
 *
 * The index is built ONCE per drag-start; each pointer move runs the
 * O(log n + k) `matchAxis` query plus a small neighbor scan for spacing.
 */

import type { BoundsMm } from "../../../rendering/types";
import {
  matchAxis,
  sortFeatures,
  type AxisFeature,
...
export interface AlignmentIndex {
  ...
  /** Non-dragged boxes, for equal-spacing detection. */
...
/**
 * Build the per-axis feature index (left/center/right → X, top/center/bottom →
 * Y) over `boxes` (these also participate in equal-spacing) plus, optionally,
 * `featureOnlyBoxes` (e.g. a board / footprint outline) that contribute
 * alignment lines but are NOT spacing neighbors.
 */
export function buildAlignmentIndexFromBoxes(input: {
  boxes: ReadonlyArray<IdBox>;
  featureOnlyBoxes?: ReadonlyArray<IdBox>;
}): AlignmentIndex {
  ...
  for (const { id, bbox } of input.boxes) {
    pushBox(id, bbox);
    boxes.push({ id, bbox });
  }
  for (const { id, bbox } of input.featureOnlyBoxes ?? []) {
    pushBox(id, bbox);
  }
...
/** Union AABB of the given boxes (drag-start group bbox). */
export function unionBoxes(
  boxes: ReadonlyArray<BoundsMm | null | undefined>,
): BoundsMm | null {
  let minX = Infinity;
  let minY = Infinity;
  let maxX = -Infinity;
