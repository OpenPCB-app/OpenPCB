    ...
    switch (g.type) {
      ...
      case "polygon": {
        for (let i = 0; i < g.points.length - 1; i++) {
          ...
          vertices.push(cur.x, cur.y, 0, next.x, next.y, 0);
        }
...
function collectFillGeometries(graphics: SymbolGraphic[]): THREE.BufferGeometry {
  ...
  return (
    <group renderOrder={RENDER_ORDER.BODIES}>
      ...
      {(strokes.attributes.position as THREE.BufferAttribute).count > 0 && (
        <lineSegments geometry={strokes} renderOrder={RENDER_ORDER.BODIES + 0.1}>
          <lineBasicMaterial
