...
function pathLength(points: Array<{ x: number; y: number }>): number {
  ...
  for (let index = 1; index < points.length; index += 1) {
    ...
    if (prev && curr) total += Math.abs(curr.x - prev.x) + Math.abs(curr.y - prev.y);
  }
  return total;
}
...
function updatePartPinsAndConnectedWires(params: {
  ...
  const { tx, designId, partId, positionNm, rotationDeg, mirrored, timestamp } = params;
  const pinRows = tx.select().from(schematicPins).where(eq(schematicPins.partId, partId)).all();
  const worlds = recomputePinWorldPositions(
    pinRows.map((pin) => ({ localPositionNm: { x: pin.localXNm, y: pin.localYNm } })),
    positionNm,
...
export function executeDesignerCommand({
  ...
  if (command.type === "create_wire") {
    const sourcePinRow = tx.select().from(schematicPins).where(and(eq(schematicPins.designId, designId), eq(schematicPins.id, command.sourcePinId))).get();
    if (!sourcePinRow) return pinNotFound(command.sourcePinId);
    const targetPinRow = tx.select().from(schematicPins).where(and(eq(schematicPins.designId, designId), eq(schematicPins.id, command.targetPinId))).get();
    if (!targetPinRow) return pinNotFound(command.targetPinId);
    const built = buildCreateWirePayload(mapPinRow(sourcePinRow), mapPinRow(targetPinRow), command.pointsNm);
    if (!built.payload) return invalidWirePath(built.invalidReason ?? "wire path is invalid");
    insertWire(tx, designId, built.payload, timestamp);
    return okResult(bumpRevision(tx, designId, revision, timestamp), built.payload.id);
  }

  if (command.type === "create_wire_junction") {
    const sourcePinRow = tx.select().from(schematicPins).where(and(eq(schematicPins.designId, designId), eq(schematicPins.id, command.sourcePinId))).get();
    if (!sourcePinRow) return pinNotFound(command.sourcePinId);
    const wireRow = tx.select().from(schematicWires).where(and(eq(schematicWires.designId, designId), eq(schematicWires.id, command.wireId))).get();
    ...
    if (!insertion) return invalidWirePath("target wire has no routable segments");
    ...
    const endpointSourcePinRow = tx.select().from(schematicPins).where(and(eq(schematicPins.designId, designId), eq(schematicPins.id, wireRow.sourcePinId))).get();
    const endpointTargetPinRow = tx.select().from(schematicPins).where(and(eq(schematicPins.designId, designId), eq(schematicPins.id, wireRow.targetPinId))).get();
    ...
    const toJunctionBuild = buildCreateWirePayload(sourcePin, pseudoJunctionPin, command.pointsNm);
    if (!toJunctionBuild.payload) return invalidWirePath(toJunctionBuild.invalidReason ?? "wire path is invalid");

    const pathToSourceEndpoint = insertion.points.slice(0, insertion.insertIndex + 1).reverse();
    const pathToTargetEndpoint = insertion.points.slice(insertion.insertIndex);
    const useSourceEndpoint = pathLength(pathToSourceEndpoint) <= pathLength(pathToTargetEndpoint);
    const endpointPath = useSourceEndpoint ? pathToSourceEndpoint : pathToTargetEndpoint;
    const targetEndpointPin = useSourceEndpoint ? mapPinRow(endpointSourcePinRow) : mapPinRow(endpointTargetPinRow);
    const finalBuild = buildCreateWirePayload(
      sourcePin,
      targetEndpointPin,
      sanitizePath([...toJunctionBuild.payload.pointsNm, ...endpointPath.slice(1)]),
    );
    if (!finalBuild.payload) return invalidWirePath(finalBuild.invalidReason ?? "wire path is invalid");

    tx.update(schematicWires)
      .set({ pointsJson: JSON.stringify(insertion.points), updatedAt: timestamp })
      .where(eq(schematicWires.id, wireRow.id))
      .run();
    insertWire(tx, designId, finalBuild.payload, timestamp);
    return okResult(bumpRevision(tx, designId, revision, timestamp), finalBuild.payload.id);
  }

  if (command.type === "move_part" || command.type === "rotate_part" || command.type === "mirror_part") {
    const partId = command.partId;
    const partRow = tx.select().from(schematicParts).where(and(eq(schematicParts.designId, designId), eq(schematicParts.id, partId))).get();
    if (!partRow) return entityNotFound(partId, "part");
    const positionNm = command.type === "move_part" ? command.positionNm : { x: partRow.positionXNm, y: partRow.positionYNm };
    const rotationDeg = command.type === "rotate_part" ? normalizeRotationDeg(command.rotationDeg) : normalizeRotationDeg(partRow.rotationDeg);
    const mirrored = command.type === "mirror_part" ? command.mirrored : partRow.mirrored === 1;
    tx.update(schematicParts)
      ...
      .run();
    updatePartPinsAndConnectedWires({ tx, designId, partId, positionNm, rotationDeg, mirrored, timestamp });
  ...
  if (command.type === "delete_entity") {
    if (command.entityKind === "part") {
      const part = tx.select({ id: schematicParts.id }).from(schematicParts).where(and(eq(schematicParts.designId, designId), eq(schematicParts.id, command.entityId))).get();
      if (!part) return entityNotFound(command.entityId, "part");
      const pinIds = tx.select({ id: schematicPins.id }).from(schematicPins).where(eq(schematicPins.partId, command.entityId)).all().map((pin) => pin.id);
      if (pinIds.length > 0) {
        tx.delete(schematicWires).where(and(eq(schematicWires.designId, designId), or(inArray(schematicWires.sourcePinId, pinIds), inArray(schematicWires.targetPinId, pinIds)))).run();
      }
      tx.delete(schematicPins).where(eq(schematicPins.partId, command.entityId)).run();
      tx.delete(schematicParts).where(eq(schematicParts.id, command.entityId)).run();
    } else if (command.entityKind === "wire") {
      const wire = tx.select({ id: schematicWires.id }).from(schematicWires).where(and(eq(schematicWires.designId, designId), eq(schematicWires.id, command.entityId))).get();
      if (!wire) return entityNotFound(command.entityId, "wire");
      tx.delete(schematicWires).where(eq(schematicWires.id, command.entityId)).run();
    } else {
      const label = tx.select({ id: schematicLabels.id }).from(schematicLabels).where(and(eq(schematicLabels.designId, designId), eq(schematicLabels.id, command.entityId))).get();
      if (!label) return entityNotFound(command.entityId, "label");
      tx.delete(schematicLabels).where(eq(schematicLabels.id, command.entityId)).run();
    }
    return okResult(bumpRevision(tx, designId, revision, timestamp), command.entityId);
  }

  const text = command.text.trim();
  if (text.length === 0) return invalidLabel("label text must not be empty");
  if (command.labelId) {
    const label = tx.select({ id: schematicLabels.id }).from(schematicLabels).where(and(eq(schematicLabels.designId, designId), eq(schematicLabels.id, command.labelId))).get();
    if (!label) return entityNotFound(command.labelId, "label");
    tx.update(schematicLabels)
      .set({ text, xNm: command.positionNm.x, yNm: command.positionNm.y, updatedAt: timestamp })
      .where(eq(schematicLabels.id, command.labelId))
      .run();
    return okResult(bumpRevision(tx, designId, revision, timestamp), command.labelId);
  }
