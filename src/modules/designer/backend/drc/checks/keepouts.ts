...
import { polylineToPolylineClosestPoints } from "../../pcb/pcb-trace-geometry";
...
function buildItems(ctx: DrcContext): CandidateItem[] {
  const referenceById = new Map(
    ctx.projection.placements.map((p) => [p.id, p.reference] as const),
  );
  const referenceOf = (placementId: string): string =>
    referenceById.get(placementId) ?? placementId;
  ...
  for (const placement of ctx.projection.placements) {
...
export function checkKeepouts(ctx: DrcContext): DrcViolationDraft[] {
  ...
  const stackup = copperLayersForCount(ctx.projection.board.layerCount);
