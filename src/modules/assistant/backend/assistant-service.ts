...
import { AssistantSettingsStore, type InternalProviderConfig } from "./settings-store";
...
export class AssistantService {
  ...
  createChat(input: { title?: string; providerConfigId?: string; provider?: AssistantProviderId; model?: string } = {}): AssistantChat {
    const appSettings = this.settings.getSettings();
    const providerConfigId = input.providerConfigId ?? input.provider ?? appSettings.defaultProviderId;
    const provider = this.requireProvider(providerConfigId);
    return this.store.createChat({ ...input, providerConfigId: provider.id, provider: provider.id, model: input.model ?? provider.defaultModel });
  }

  async submitMessage(chatId: string, input: SubmitAssistantMessageInput): Promise<SubmitAssistantMessageResult> {
    ...
    const providerConfigId = input.providerConfigId ?? input.provider ?? chat.providerConfigId;
    ...
    chat = this.store.updateChatModel(chatId, provider, model, providerConfig.id);
    const userMessage = this.store.createMessage({ chatId, role: "user", content: input.content });
    const assistantMessage = this.store.createMessage({ chatId, role: "assistant", content: "" });
    const result = await this.tasks.createTask({
      type: "assistant.chat",
      queueKey: "assistant",
      payload: { chatId, assistantMessageId: assistantMessage.id, providerConfigId: providerConfig.id, provider, model },
      correlation: { scopeId: chatId },
      tags: ["assistant", providerConfig.id],
    });
    this.store.setMessageTask(assistantMessage.id, result.task.id);
    return { chat, userMessage, assistantMessage: this.store.getMessage(assistantMessage.id) ?? assistantMessage, taskId: result.task.id };
  }
  ...
  async approveToolEvent(eventId: string): Promise<{ ok: true; followUpTaskId: string }> {
    ...
    const result = await tool.execute(event.argsJson ? JSON.parse(event.argsJson) as unknown : {});
    this.store.updateToolEventResult(eventId, JSON.stringify(result), false);
    const chat = this.store.getChat(event.chatId);
    const followUp = this.store.createMessage({ chatId: event.chatId, role: "tool", content: JSON.stringify({ tool: event.toolName, result }) });
    const providerConfig = this.requireUsableProvider(chat?.providerConfigId ?? "openai");
    const resultTask = await this.tasks.createTask({
      type: "assistant.chat",
      queueKey: "assistant",
      payload: { chatId: event.chatId, assistantMessageId: event.assistantMessageId, providerConfigId: providerConfig.id, provider: providerConfig.id, model: chat?.model ?? providerConfig.defaultModel },
  ...
  rejectToolEvent(eventId: string): void {
    const event = this.store.getToolEvent(eventId);
    if (!event) throw new Error(`Tool event not found: ${eventId}`);
    this.store.updateToolEventResult(eventId, JSON.stringify({ rejected: true }), false);
    this.store.createMessage({ chatId: event.chatId, role: "tool", content: JSON.stringify({ tool: event.toolName, error: "User rejected" }) });
  }

  private registerExecutor(): void {
    this.tasks.registerExecutor("assistant.chat", {
      execute: async (taskCtx) => {
        const payload = taskCtx.task.payload as { chatId: string; assistantMessageId: string; providerConfigId?: string; provider: AssistantProviderId; model: string };
        const providerConfig = this.requireUsableProvider(payload.providerConfigId ?? payload.provider);
        const provider = this.createProvider(providerConfig);
        const toolDefs = this.tools.list().map((tool) => tool.definition);
        const maxToolIterations = 5;

        for (let iteration = 0; iteration < maxToolIterations; iteration++) {
          const messages = this.buildProviderMessages(payload.chatId, payload.assistantMessageId);
          const response = await provider.stream({
            ...
            onToken: async (token) => {
              this.store.appendMessageContent(payload.assistantMessageId, token);
              await taskCtx.emitChunk({ content: token, kind: "text" });
            },
          });

          if (response.toolCalls.length === 0) {
            return { content: response.content, messageId: payload.assistantMessageId };
          }

          const pending: AIToolCall[] = [];
          for (const call of response.toolCalls) {
            const tool = this.tools.get(call.name);
            await this.persistToolEvent(payload.chatId, payload.assistantMessageId, taskCtx.task.id, {
              kind: "tool_call", toolCallId: call.id, toolName: call.name, argsJson: call.argumentsJson,
            });
            if (!tool) {
              this.store.createMessage({ chatId: payload.chatId, role: "tool", content: JSON.stringify({ error: `Tool ${call.name} not found` }) });
              continue;
            }
            if (requiresConfirmation(this.settings.getSettings().toolExecutionPolicy, tool.effect)) {
              pending.push(call);
              const notice = `\n\nTool ${call.name} requires confirmation before execution.`;
              this.store.appendMessageContent(payload.assistantMessageId, notice);
              await taskCtx.emitChunk({ content: JSON.stringify({ kind: "confirmation_required", tool: call.name, callId: call.id }), kind: "json" });
              await this.persistToolEvent(payload.chatId, payload.assistantMessageId, taskCtx.task.id, {
                kind: "confirmation_required", toolCallId: call.id, toolName: call.name,
              });
              continue;
            }
            const result = await tool.execute(call.argumentsJson ? JSON.parse(call.argumentsJson) as unknown : {});
            const toolMessage = this.store.createMessage({ chatId: payload.chatId, role: "tool", content: JSON.stringify({ tool: call.name, result }) });
            await this.persistToolEvent(payload.chatId, payload.assistantMessageId, taskCtx.task.id, {
              kind: "tool_result", toolCallId: call.id, toolName: call.name, resultJson: JSON.stringify(result),
            });
            await taskCtx.emitEvent({ type: "task.progress", data: { kind: "tool_result", callId: call.id, messageId: toolMessage.id } });
          }

          if (pending.length > 0) {
            return { content: response.content, messageId: payload.assistantMessageId, pendingToolCalls: pending };
          }
        }

        return { content: "Reached maximum tool iterations.", messageId: payload.assistantMessageId };
      },
    });
  }
  ...
  createProviderConfig(input: AssistantProviderConfigInput): AssistantProviderConfig {
    return this.settings.createProvider(input);
  }

  updateProviderConfig(id: string, input: AssistantProviderConfigInput): AssistantProviderConfig {
  ...
  async refreshProviderModels(providerId: string): Promise<AssistantProviderModel[]> {
  ...
  async testProvider(providerId: string, input: { includeCompletion?: boolean } = {}): Promise<ProviderTestResult> {
    const provider = this.requireUsableProvider(providerId);
    ...
    const testModel = modelIds.includes(provider.defaultModel) ? provider.defaultModel : modelIds[0];
    if (!testModel) throw new ValidationError(`Provider ${provider.label} did not return any models`);
    ...
    return {
      ...
      message: input.includeCompletion ? `Models listed and completion accepted with ${testModel}.` : "Models listed successfully.",
    };
  }
  ...
  private requireProvider(providerId: string): InternalProviderConfig {
    ...
    if (!provider) throw new ValidationError(`Provider not found: ${providerId}`);
    return provider;
  }

  private createProvider(provider: InternalProviderConfig): OpenAICompatibleProvider {
    return new OpenAICompatibleProvider(provider.id, { baseUrl: provider.baseUrl, apiKey: provider.apiKey ?? undefined });
  }

  private buildProviderMessages(chatId: string, excludeMessageId: string): AIProviderRequest["messages"] {
  ...
  private async persistToolEvent(
    ...
    const rawSql = (this.ctx.db as { rawSql<T = unknown>(query: string, params?: unknown[]): T[] }).rawSql.bind(this.ctx.db);
    rawSql("INSERT INTO assistant_task_tool_event (id,chat_id,assistant_message_id,task_id,seq,kind,tool_call_id,tool_name,args_json,result_json,is_error,created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [row.id, row.chatId, row.assistantMessageId, row.taskId, row.seq, row.kind, row.toolCallId, row.toolName, row.argsJson, row.resultJson, row.isError, row.createdAt]);
  }
}
