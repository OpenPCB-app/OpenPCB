...
import { components, footprints } from "../schema";
...
import { decodeTextEntry, extractZipEntries, type ZipEntryContent } from "./archive/extract-zip";

type Confidence = "high" | "medium" | "low";

interface ModelSelection {
  fileName: string;
  association: "footprint-model-ref" | "symbol-name" | "archive-name" | "single-model";
}
...
function basenameFromFootprintProperty(value: string | undefined): string | null {
...
function detectProvider(
  ...
  if (hasSnapedaDoc || propertyBlob.includes("snapeda") || propertyBlob.includes("snapmagic")) {
    return "snapeda";
  }
...
function pinPadCompatible(
  ...
  const padNumbers = new Set(footprint.preview.pads.map((pad) => pad.number.trim()));
...
function chooseSymbol(
  ...
  if (symbols.length === 0) {
    throw new ImportValidationError("ZIP archive does not contain a KiCad symbol");
  }
  ...
  for (const symbol of symbols) {
    ...
    if (normalizeName(symbol.name).includes(archiveKey) && archiveKey.length > 0) score += 20;
    if (score > bestScore) {
      best = symbol;
      bestScore = score;
    }
  }
  return { symbol: best, confidence: bestScore >= 80 ? "high" : bestScore >= 40 ? "medium" : "low" };
}
...
function chooseFootprint(
  ...
  archiveBase: string,
): { footprint: NormalizedImportedFootprint; confidence: Confidence; warnings: ArchiveImportWarning[] } {
  if (footprintsList.length === 0) {
    throw new ImportValidationError("ZIP archive does not contain a KiCad footprint");
  }
  ...
  for (const footprint of footprintsList) {
    ...
    if (expectedFootprint && (footprintKey === expectedFootprint || fileKey === expectedFootprint)) {
      score += 120;
    }
    if (pinPadCompatible(symbol, footprint)) score += 60;
    if (archiveKey && (footprintKey.includes(archiveKey) || fileKey.includes(archiveKey))) score += 15;
    if (score > bestScore) {
      best = footprint;
      bestScore = score;
    }
  ...
  if (!expectedFootprint) {
    warnings.push(warning("missing_symbol_footprint_property", "Symbol has no Footprint property; footprint was selected by compatibility heuristics"));
  } else if (bestScore < 120) {
    warnings.push(warning("footprint_property_not_matched", "Symbol Footprint property did not exactly match an imported footprint; selected a compatible footprint"));
  }

  const confidence: Confidence = bestScore >= 140 ? "high" : bestScore >= 80 ? "medium" : "low";
...
function chooseModel(
  ...
  const byBase = new Map(models.map((entry) => [entry.baseName.toLowerCase(), entry]));
  for (const ref of rawFootprint.model3dRefs) {
    const match = byBase.get(ref.resolvedFileName.toLowerCase());
    if (match) {
      return {
        selection: { fileName: match.baseName, association: "footprint-model-ref" },
        warnings,
      };
    }
  }
  ...
  const symbolMatch = models.find((entry) => normalizeName(entry.baseName) === symbolKey);
  if (symbolMatch) {
    warnings.push(warning("model_associated_by_symbol_name", "3D model was associated by symbol/archive filename because footprint has no matching model reference"));
    return { selection: { fileName: symbolMatch.baseName, association: "symbol-name" }, warnings };
  }

  const archiveKey = normalizeName(archiveBase);
  const archiveMatch = models.find((entry) => normalizeName(entry.baseName) === archiveKey);
  if (archiveMatch) {
    warnings.push(warning("model_associated_by_archive_name", "3D model was associated by archive filename because footprint has no matching model reference"));
    return { selection: { fileName: archiveMatch.baseName, association: "archive-name" }, warnings };
  }

  if (models.length === 1) {
    warnings.push(warning("model_associated_as_single_candidate", "Only one 3D model was found; associated it with the selected footprint"));
    return { selection: { fileName: models[0]!.baseName, association: "single-model" }, warnings };
  }

  warnings.push(warning("model_not_selected", "Multiple 3D models were found, but none matched the selected footprint"));
...
export async function commitKicadZipImport(
  ...
  const legacySymbols = entries.filter((entry) => entry.extension === ".lib" || entry.extension === ".dcm");
  if (legacySymbols.length > 0) {
    throw new ImportValidationError("Legacy KiCad .lib/.dcm archives are not supported yet. Please use KiCad v6+ .kicad_sym ZIPs.");
  }

  const symbolEntries = entries.filter((entry) => entry.extension === ".kicad_sym");
  const footprintEntries = entries.filter((entry) => entry.extension === ".kicad_mod");
  const modelEntries = entries.filter((entry) => MODEL_EXTENSIONS.has(entry.extension));

  if (symbolEntries.length === 0) throw new ImportValidationError("ZIP archive does not contain a .kicad_sym file");
  if (footprintEntries.length === 0) throw new ImportValidationError("ZIP archive does not contain a .kicad_mod file");
  ...
  const parsed = parseImportBundle({ symbolLibrary, footprints: footprintsPayload });
  ...
  const footprintProperty = basenameFromFootprintProperty(rawSymbol?.properties.Footprint);
  ...
  if (!rawFootprint) throw new ImportValidationError("Failed to map selected footprint raw payload");

  const modelChoice = chooseModel(modelEntries, rawFootprint, symbolChoice.symbol.name, archiveBase);
  const warnings = [...parsed.warnings.map((item) => warning(item.code, item.message)), ...footprintChoice.warnings, ...modelChoice.warnings];
  const provider = detectProvider(entries, rawSymbol?.properties ?? null);
  const confidence = mergeConfidence(symbolChoice.confidence, footprintChoice.confidence);

  if (confidence !== "high") {
    warnings.push(warning("low_confidence_selection", `ZIP import selected symbol/footprint with ${confidence} confidence`));
  }
