    ...
    const initialize = async () => {
      ...
      try {
        ...
        if (!healthResponse.ok) {
          throw new Error(
            `Backend health check failed: HTTP ${healthResponse.status}`,
          );
        }
        if (!registryResponse.ok) {
          throw new Error(
            `Module registry bootstrap failed: HTTP ${registryResponse.status}`,
          );
        }

        const moduleRegistry =
          (await registryResponse.json()) as ModuleRegistryResponse;
