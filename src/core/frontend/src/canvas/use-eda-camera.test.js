...
describe("use-eda-camera wheel policy", () => {
  ...
  it("detects likely trackpad pixel scroll and ignores pinch gestures", () => {
    ...
    expect(
      isLikelyTrackpadWheelEvent(wheelEvent({ deltaY: 3, deltaMode: 0, ctrlKey: true })),
    ).toBe(false);
    expect(
      isLikelyTrackpadWheelEvent(wheelEvent({ deltaY: 3, deltaMode: 0, metaKey: true })),
    ).toBe(false);
    expect(
  ...
  it("normalizes zoom deltas with correct sign and pinch amplification", () => {
    expect(normalizeZoomDelta(wheelEvent({ deltaY: -100, deltaMode: 0 }))).toBeGreaterThan(0);
    expect(normalizeZoomDelta(wheelEvent({ deltaY: 100, deltaMode: 0 }))).toBeLessThan(0);
  ...
  it("keeps pan normalization deterministic for line and page modes", () => {
    expect(normalizePanDelta(wheelEvent({ deltaX: 1, deltaY: 2, deltaMode: 0 }))).toEqual({
      dx: 1,
      dy: 2,
    });
    expect(normalizePanDelta(wheelEvent({ deltaX: 1, deltaY: 2, deltaMode: 1 }))).toEqual({
      dx: 40,
      dy: 80,
    });
    expect(normalizePanDelta(wheelEvent({ deltaX: 1, deltaY: 2, deltaMode: 2 }))).toEqual({
      dx: 800,
      dy: 1600,
    });
  });
