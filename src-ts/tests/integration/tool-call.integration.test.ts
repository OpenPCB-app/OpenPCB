...
import { TaskExecutor, type ExecutionEvent } from '../../src/domain/services/queue/task-executor';
...
import { initializeChatManager, type ChatManager, type ContextMessage } from '../../src/domain/services/chat-manager';
...
describe('Tool call integration', () => {
    ...
    beforeAll(async () => {
        dbDir = mkdtempSync(join(tmpdir(), 'onemind-tool-call-'));
    ...
    it('should emit tool_result and persist ordered tool events without tool-role messages', async () => {
        ...
        const mockEngine: KernelProviderEngine = {
            ...
            getModel: () => undefined,
            chat: async () => ({ text: '' }),
            stream: async (_request: unknown, callbacks: StreamCallbacks): Promise<ChatResult> => {
        ...
        const queueManager = new TaskQueueManager({ debug: false, agingIntervalMs: 60000 });
        ...
        try {
            ...
            expect(events.some((event) => event.type === 'task.tool_call')).toBe(true);
            ...
            const callEvent = persistedEvents.find((event) => event.kind === "tool-call");
            ...
            const toolResultContext = followupMessages.some((message) =>
    ...
    it('should route MCP tool calls through ToolDispatcher and keep follow-up cycle intact', async () => {
        ...
        if (!mcpToolDefinition) {
            throw new Error('Expected MCP tool definition mcp.knowledge.fetch.info to be registered');
        }
        ...
        try {
            ...
            const toolCallEvent = executionEvents.find((event) => event.type === 'task.tool_call');
            const toolResultEvent = executionEvents.find((event) => event.type === 'task.tool_result');
            const completedEventIndex = executionEvents.findIndex((event) => event.type === 'task.completed');
            const toolResultIndex = executionEvents.findIndex((event) => event.type === 'task.tool_result');
            ...
            expect(persistedEvents[0]?.toolName).toBe(mcpToolDefinition.function.name);
            expect(persistedEvents[1]?.kind).toBe('tool-result');
            expect(persistedEvents[1]?.toolName).toBe(mcpToolDefinition.function.name);
            ...
            expect(followupSpec.assistantMessageId).toBe(mcpTask.assistantMessageId ?? undefined);

            const followupMessages = followupSpec.messages ?? [];
            const hasMcpResultContext = followupMessages.some((message) =>
                message.content.includes('Tool result') &&
