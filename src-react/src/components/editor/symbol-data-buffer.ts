import { parseKicadSymbolImport, type ComponentType } from "@/lib/api/component-api";
...
function toBackendGraphic(value: unknown): BackendSymbolGraphic | null {
    ...
    switch (value.type) {
        case "line": {
            ...
            if (x1 === null || y1 === null || x2 === null || y2 === null || strokeWidth === null) {
                return null;
            }
        ...
        case "circle": {
            ...
            const filled = toBoolean(value.filled);
            const strokeWidth = toNumber(value.strokeWidth);
            if (cx === null || cy === null || radius === null || filled === null || strokeWidth === null) {
                return null;
            }
        ...
        case "polygon": {
            ...
            const strokeWidth = toNumber(value.strokeWidth);
            if (points === null || filled === null || closed === null || strokeWidth === null) {
                return null;
            }
        ...
        case "text": {
            ...
            if (x === null || y === null || content === null || fontSize === null || rotation === null) {
                return null;
            }
...
function backendGraphicToEditor(graphic: BackendSymbolGraphic, index: number): SymbolGraphic {
    const base = { id: `component-symbol-${index}`, zIndex: index };
    return { ...graphic, ...base };
}

function toBodyBounds(graphics: SymbolGraphic[]): { minX: number; minY: number; maxX: number; maxY: number } | null {
...
function toFallbackPins(
    ...
    const left: Array<{ index: number; name: string; electricalType: SymbolPin["electricalType"] }> = [];
    const right: Array<{ index: number; name: string; electricalType: SymbolPin["electricalType"] }> = [];
    ...
    pinDefinitions.forEach((pin, index) => {
        ...
        if (pin.electricalType === "output" || pin.electricalType === "open_collector" || pin.electricalType === "open_emitter") {
            right.push(entry);
            return;
        }
        left.push(entry);
    ...
    const layout = (
        sidePins: Array<{ index: number; name: string; electricalType: SymbolPin["electricalType"] }>,
        side: SymbolPin["side"],
    ): SymbolPin[] => {
        ...
        return sidePins.map((pin, lineIndex) => ({
            ...
            position: {
                x: side === "left" ? bodyMinX - DEFAULT_PIN_LENGTH : bodyMaxX + DEFAULT_PIN_LENGTH,
...
function createFallbackDraftFromSymbolData(component: ComponentType): SymbolDraft {
    ...
    const width = Math.max(DEFAULT_BODY_WIDTH, bounds ? bounds.maxX - bounds.minX : DEFAULT_BODY_WIDTH);
    const height = Math.max(DEFAULT_BODY_HEIGHT, bounds ? bounds.maxY - bounds.minY : DEFAULT_BODY_HEIGHT);
    ...
    return {
        ...
        importPreservation: symbolData.rawKicadSource
            ? {
                  rawSource: symbolData.rawKicSource,
                  sourceFileName: null,
                  warnings: [],
...
function transformBodyPresetToGraphics(body: SymbolDraft["body"]): BackendSymbolGraphic[] {
...
function transformDraftGraphics(graphics: SymbolDraft["graphics"]): BackendSymbolGraphic[] {
...
export async function loadSymbolDraftFromComponent(component: ComponentType): Promise<{ draft: SymbolDraft; warning: string | null }> {
    if (component.symbolData.rawKicadSource) {
        try {
            const parsed = await parseKicadSymbolImport(component.symbolData.rawKicadSource);
            const parsedDraft = convertParsedKicadSymbolToDraft(parsed.symbol, component.displayLabel);
            return {
                draft: {
                    ...parsedDraft,
                    metadata: {
                        ...
                        name: component.displayLabel,
                        description: component.description,
                        referencePrefix: component.symbolData.referencePrefix || parsedDraft.metadata.referencePrefix,
                    },
                },
                warning: null,
            };
            ...
            return {
                ...
                warning: "Unable to parse saved KiCad symbol source. Loaded fallback editable symbol layout.",
            };
        }
    }
...
export function transformSymbolDraftToComponentSymbolData(
    ...
    const referencePrefix = draft.metadata.referencePrefix.trim().toUpperCase().slice(0, 5) || "U";
    ...
    return {
        ...
        unitCount: draft.importPreservation?.unitCount ?? existingSymbolData?.unitCount ?? 1,
        bodyGraphics: [
            ...transformBodyPresetToGraphics(draft.body),
            ...transformDraftGraphics(draft.graphics),
        ],
        rawKicadSource: draft.importPreservation?.rawSource ?? existingSymbolData?.rawKicadSource ?? null,
