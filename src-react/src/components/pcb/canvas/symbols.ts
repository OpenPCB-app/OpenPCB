...
function getTwoTerminalMetrics(symbol: SymbolEntity): TwoTerminalMetrics {
    const firstPin = symbol.pins[0]?.position ?? { x: -DEFAULT_PIN_SPAN / 2, y: 0 };
...
function getRectBodyLocalBounds(symbol: SymbolEntity): Bounds {
    ...
    const height = extents
        ? Math.max(extents.maxY - extents.minY + RECT_PADDING_Y * 2, MIN_RECT_HEIGHT)
        : MIN_RECT_HEIGHT;
...
function getSinglePinBodyLocalBounds(symbol: SymbolEntity, width: number, height: number): Bounds {
...
function applySymbolTransform(
    ...
    const origin = schematicToScreen(symbol.position.x, symbol.position.y, viewport);
...
function strokeTwoTerminalLeads(ctx: CanvasRenderingContext2D, metrics: TwoTerminalMetrics): void {
    ctx.beginPath();
...
function renderResistor(ctx: CanvasRenderingContext2D, symbol: SymbolEntity): void {
    const metrics = getTwoTerminalMetrics(symbol);
...
function renderCapacitor(ctx: CanvasRenderingContext2D, symbol: SymbolEntity): void {
    const metrics = getTwoTerminalMetrics(symbol);
...
function renderInductor(ctx: CanvasRenderingContext2D, symbol: SymbolEntity): void {
    const metrics = getTwoTerminalMetrics(symbol);
...
function renderDiode(ctx: CanvasRenderingContext2D, symbol: SymbolEntity, led: boolean): void {
    const metrics = getTwoTerminalMetrics(symbol);
    const lineX = metrics.bodyRight - (metrics.bodyRight - metrics.bodyLeft) * 0.18;
    ...
    ctx.moveTo(metrics.bodyRight - 80_000, metrics.bodyTop - arrowOffset - arrowLength);
    ctx.lineTo(metrics.bodyRight, metrics.bodyTop - arrowOffset - arrowLength);
    ctx.lineTo(metrics.bodyRight - 10_000, metrics.bodyTop - arrowOffset - 80_000);

    ctx.moveTo(metrics.bodyRight - arrowLength * 1.3, metrics.bodyTop + 40_000);
    ctx.lineTo(metrics.bodyRight - 40_000, metrics.bodyTop - arrowLength + 40_000);
    ctx.moveTo(metrics.bodyRight - 120_000, metrics.bodyTop - arrowLength + 40_000);
    ctx.lineTo(metrics.bodyRight - 40_000, metrics.bodyTop - arrowLength + 40_000);
    ctx.lineTo(metrics.bodyRight - 50_000, metrics.bodyTop - 40_000);
    ctx.stroke();
}

function renderGround(ctx: CanvasRenderingContext2D, symbol: SymbolEntity): void {
    const pin = symbol.pins[0]?.position ?? { x: 0, y: 0 };
...
function renderPower(ctx: CanvasRenderingContext2D, symbol: SymbolEntity): void {
    const pin = symbol.pins[0]?.position ?? { x: 0, y: 0 };
...
function renderOpAmp(ctx: CanvasRenderingContext2D, symbol: SymbolEntity): void {
...
function renderRectangularSymbol(ctx: CanvasRenderingContext2D, symbol: SymbolEntity): void {
    drawRect(ctx, getRectBodyLocalBounds(symbol));
}

function renderBjt(ctx: CanvasRenderingContext2D, symbol: SymbolEntity, inwardArrow: boolean): void {
    const center = getLocalCenter(symbol);
...
function renderMosfet(ctx: CanvasRenderingContext2D, symbol: SymbolEntity, pChannel: boolean): void {
    const center = getLocalCenter(symbol);
...
function renderPins(
    ...
    const connectorRadius = CONNECTOR_RADIUS_PX / Math.max(viewport.zoom, Number.EPSILON);
