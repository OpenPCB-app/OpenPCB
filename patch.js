...
let content = fs.readFileSync('src-react/src/components/symbol-editor/kicad-import.ts', 'utf-8');
...
content = content.replace(
  /function isRectangularTwoSidedImportedIcLayout\([\s\S]*?\}[\s\S]*?return hasLeft && hasRight;\n\}/,
...
content = content.replace(
  /function normalizeRectangularTwoSidedImportedIcLayout\([\s\S]*?\}\n\}\n/,
  ...
  const normalizedGraphics = graphics.map((graphic) => {
    switch (graphic.type) {
      ...
      case "bezier":
        return {
          ...graphic,
          points: graphic.points.map((p) => ({ x: p.x * scale, y: p.y * scale })) as typeof graphic.points,
        };
      case "text":
        return {
          ...graphic,
...
content = content.replace(
  /function normalizeImportedSchematicContent\([\s\S]*?\}\n\}\n/,
...
fs.writeFileSync('src-react/src/components/symbol-editor/kicad-import.ts', content, 'utf-8');
